<template>
  <div class="endereco">
    ...
    <div v-if="enderecoSalvo && !mostrarFormulario" class="endereco-salvo">
      ...
      <p>{{ enderecoSalvo.bairro }} - {{ enderecoSalvo.city }}/{{ enderecoSalvo.estado }}</p>
    ...
    <form v-else @submit.prevent="salvarEndereco" class="endereco-form">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" id="rua" v-model="endereco.rua" placeholder="Name da rua" :class="{ 'is-invalid': v$.endereco.rua.$error }" />
        </div>
        <div class="form-group">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" id="bairro" v-model="endereco.bairro" placeholder="Name do bairro" :class="{ 'is-invalid': v$.endereco.bairro.$error }" />
        </div>
        <div class="form-group">
          <label for="city">Cidade:</label>
          <input type="text" id="city" v-model="endereco.city" placeholder="Name da city" :class="{ 'is-invalid': v$.endereco.city.$error }" />
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
...
onMounted(async () => {
  ...
  try {
    ...
    const possuiEnderecoSalvo = data.endereco &&
      ['cep', 'rua', 'numero', 'bairro', 'city', 'estado'].every((campo) => !!data.endereco[campo]);
    ...
    console.error("Erro ao carregar data do usuário:", err);
  }
});
...
const salvarEndereco = async () => {
  ...
  if (currentUser) {
    try {
      await userService.atualizar(currentUser.id, { endereco: { ...endereco.value } });
