<template>
  <div class="confirmacao">
    <h2>Confirmação do Order</h2>

    <div class="resumo-order">
      <h3>Resumo do Order</h3>

      <div class="itens-order">
        <div v-for="item in itensSacola" :key="item.id" class="item-resumo">
          <div class="item-info">
            <img :src="item.imagens[0].url" :alt="item.name" class="item-imagem" />
    ...
    <div class="endereco-entrega">
      ...
      <div class="endereco-info">
        ...
        <p>{{ endereco.bairro }}, {{ endereco.city }} - {{ endereco.estado }}</p>
    ...
    <div class="acoes-confirmacao">
      ...
      <button @click="finalizarOrder" :disabled="loading" class="btn-finalizar">
        <span v-if="loading">Processando...</span>
        <span v-else>Finalizar Order</span>
      </button>
    </div>
  </div>
</template>
...
const finalizarOrder = async () => {
  if (!endereco.value || !pagamento.value) {
    alert('Data de endereço ou pagamento incompletos');
    return;
  }
  ...
  try {
    ...
    if (!user) {
      alert('Você precisa estar logado para finalizar o order');
      router.push('/perfil');
      return;
    }
    ...
    const orderCriado = await orderService.criar(order);
    ...
    try {
      ...
      alert('Order criado, mas houve erro ao atualizar o estoque. Entre em contato com o suporte.');
    }

    checkoutStore.setOrder(orderCriado);
    sacolaStore.limparSacola();

    alert('Order realizado com sucesso!');
    router.push('/perfil/my-orders');
  } catch (error) {
    console.error('Erro ao finalizar order:', error);
    alert('Erro ao finalizar order. Tente novamente.');
  } finally {
    loading.value = false;
  }
};
